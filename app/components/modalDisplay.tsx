...
export default function ModalDisplayRecipe({
  ...
  return (
    <div
      className={`${
        isRecipeModalOpen ? "z-30" : "hidden"
      } bg-black bg-opacity-45 overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 flex justify-center items-center w-full md:inset-0 h-full max-h-full`}
    >
      <div className="absolute z-2 w-full h-full" onClick={closeDropdown}></div>
      <Button
        text={<X className="h-12 w-12" />}
        classname="fixed top-2 left-2 md:left-10 lg:left-16 bg-transparent hover:bg-transparent shadow-none"
